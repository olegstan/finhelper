...
export default class Trade extends BaseModel {
  ...
  static getById(id, callback) {
    Api.get('active-trade', 'index').where('id', id).with('currency').with('to_account', 'to_account.currency', 'to_account.user_account').with('from_account', 'from_account.currency', 'from_account.user_account').with('get_account', 'get_account.currency').with('payments', 'payments.currency').with('commissions', 'commissions.currency').with('active', 'active.item', 'active.sell_trades', 'active.buy_trades').orderBy('trade_at', 'DESC').first(callback);
  }
}